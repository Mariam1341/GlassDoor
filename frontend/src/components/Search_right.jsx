...
const SearchRight = ({ company, jobProfile, location, salaryRange, btnStatus, jobId }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (!token) {
        setModalStatus({ isOpen: true, message: 'No authentication token found. Please log in.' });
        setTimeout(() => {
          setModalStatus({ isOpen: false, message: '' });
          history.push('/SignIn');
        }, 3000);
        return;
      }
      ...
      try {
        ...
        console.log('SearchRight - User data:', response.data);
        setUserData(response.data);
      } catch (err) {
        console.error('Failed to fetch user data:', err.response?.status, err.response?.data);
        setModalStatus({ isOpen: true, message: 'Failed to fetch user profile. Please log in again.' });
  ...
  const handleApply = async () => {
    if (btnTag === 'Applied') {
      setModalStatus({ isOpen: true, message: 'You have already applied for this job!' });
      handleHideModal();
      return;
    }

    if (!userData) {
      setModalStatus({ isOpen: true, message: 'User data not loaded. Please try again.' });
      handleHideModal();
      return;
    }
    ...
    try {
      ...
      const jobApplication = {
        ...
        qualification: userData.profile?.qualification || 'N/A',
        resumeLink: userData.profile?.resumeUrl || 'N/A',
        status: 'Applied',
        skills: userData.profile?.skills || [],
        jobId: jobId, // Dynamic jobId from props
        userId: userData.id || userData._id // MongoDB uses _id
      };
      ...
      if (response.status === 200 || response.status === 201) {
        setBtnTag('Applied');
        setModalStatus({ isOpen: true, message: 'Successfully applied for this job!' });
        handleHideModal();
      }
    } catch (error) {
      console.error('Error applying for job:', error.response?.status, error.response?.data);
      setModalStatus({ isOpen: true, message: 'Failed to apply for this job. Please try again.' });
      handleHideModal();
    }
  };
        ...
        <div className={styles.searchRight_div1_3}>
          <div className={styles.searchRight_div1_3_col}>
            ...
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Compensation & Benefits:</strong> N/A
            </div>
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Culture & Values:</strong> N/A
            </div>
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Career Opportunities:</strong> N/A
            </div>
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Work/Life Balance:</strong> N/A
            </div>
          </div>
          <div className={styles.searchRight_div1_3_col}>
            ...
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Job Function:</strong> Game Developer
            </div>
            <div className={styles.searchRight_div1_3_col_div}>
              <strong>Industry:</strong> N/A
            </div>
            <div className={styles.searchRight_div1_3_col_div}>
